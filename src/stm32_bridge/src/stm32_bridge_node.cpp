...
#include <stm32_bridge/INTAKEPositionCmd.h>
#include <stm32_bridge/EXCAVATESpeedCmd.h>
...
void intake_position_callback(const stm32_bridge::INTAKEPositionCmd::ConstPtr& msg);
void excavate_speed_callback(const stm32_bridge::EXCAVATESpeedCmd::ConstPtr& msg);
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber intake_position_sub = nh->subscribe("intake/position", 5, intake_position_callback);
  ros::Subscriber excavate_speed_sub = nh->subscribe("excavate/speed", 5, excavate_speed_callback);
  // Publishers
...
void intake_position_callback(const stm32_bridge::INTAKEPositionCmd& msg) {
...
void excavate_speed_callback(const stm32_bridge::EXCAVATESpeedCmd& msg) {
  uint8_t buf[4];
  int idx = 0;
  buffer_put_int32(buf, &idx, msg.speed);
  can_msgs::Frame my_frame = make_frame(102, 50, buf, 4);
